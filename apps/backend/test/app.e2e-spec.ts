...
describe('AppController (e2e)', () => {
  ...
  it('/ (GET)', async () => {
    ...
    expect(body.name).toBe('baby_app_backend');
  ...
  it('/children (GET) requires authentication', async () => {
    await request(app.getHttpServer()).get('/children').expect(401);
  });

  it('/timestamps (GET) is public for the POC', async () => {
    const response = await request(app.getHttpServer())
      .get('/timestamps')
      .expect(200);
  ...
  it('/api/auth/sign-in/anonymous (POST) creates an authenticated anonymous session', async () => {
    ...
    await agent.get('/children').expect(200);
    await agent
  ...
  it('/api-json (GET)', async () => {
    ...
    expect(body.paths).toHaveProperty('/children');
