...
Commit `docs/openapi.json` whenever endpoint/DTO changes alter the generated contract.
...
7. Add or update matching Bruno requests under `bruno/collections/baby-app-backend/` for local manual testing.
8. Run `pnpm run openapi:generate`.
9. Check `docs/openapi.json` or `/api-json` and confirm the schema includes the route and DTO models needed by client apps.
